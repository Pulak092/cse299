...
if(isset($_POST['Done']))
{
    ...
    $check = $mysqli->query("SELECT * FROM expert WHERE Email = '$ma'");
    ...
    else
    {
        ...
        {
            $vkey = rand();
            $otp = $mysqli->query("UPDATE `expert` SET `expert`.`vk` = $vkey WHERE Email = '$ma' ;");
            

            if($otp)
            {
                $check1 = $mysqli->query("SELECT * FROM expert WHERE Email = '$ma'");
