...
$sql = $mysqli->query("SELECT * FROM investor WHERE `investor`.`Email` = '$Iv'");
...
if(isset($_POST['submit']))
{
    ...
    $insert = $mysqli->query("UPDATE `investor` SET `Image` = '$folder' WHERE `investor`.`Email` = '$Iv'");
        if($insert)
        {
            echo '<script type ="text/JavaScript">';  
            echo 'alert("Your profile picture Updated successfully")';  
            echo '</script>';
            header("Refresh:0; url=Ip.php");
        }
        else {
