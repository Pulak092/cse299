...
if(isset($_POST['Done']))
{
    ...
    $check = $mysqli->query("SELECT * FROM entraprenour WHERE Email = '$ma'");
    ...
    else
    {
        ...
        {
            $vkey = rand();
            $otp = $mysqli->query("UPDATE `entraprenour` SET `entraprenour`.`vk` = $vkey WHERE Email = '$ma' ;");
            

            if($otp)
            {
                $check1 = $mysqli->query("SELECT * FROM entraprenour WHERE Email = '$ma'");
